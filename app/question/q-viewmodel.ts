...
import { ItemStatus, PrivacyLevel, Questions } from '~/common/items';
...
export class QViewModel extends Observable {
  ...
  private _itemsPerPage: number | 10 | 20 | 50 = 50;
  ...
  private async applyFiltersAndSort(): Promise<void> {
    ...
    if (this.totalItems <= 50) { this._itemsPerPage = 10; }
    else if (this.totalItems > 50 && this.totalItems <= 100) { this._itemsPerPage = 20; }
    else { this._itemsPerPage = 50; }
    
    const findhtml = filtered.filter(item => item.type === 'html');
    var change = false;
    // console.log(findhtml.length);
    findhtml.forEach(function(item, index) {
      if (item.type === 'html') { change = true; } else { change = false; } 
    });

    if (!change) { this.backgroundColor = "#ff0000"; this.label1Visibility = "visible"; }
    else { this.backgroundColor = "#e0e0e0"; this.label1Visibility = "collapse"; }
    ...
    // console.log(this.totalItems, stop)
  ...
  getItemsByStatus(status: ItemStatus): Array<Questions> {
    return Array.from(this._items.filter(item => item.status === status));
  }

  searchItems(query: string): Array<Questions> {
